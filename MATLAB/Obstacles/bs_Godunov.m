...
%   
%   Os testes suportados por esse programa sao: Escoamento sobre osbstaculo
%   trapezoidal em leito seco e os demais testes ja executados a menos de 
%   mudancas nas condicoes inicias e de contorno.
tic
...
%Constantes
g   = 9.8;  cr   = 0.3; ntmax   = 10e6; 
tol = 1e-6; nint = 50;  epsilon = 1e-4;

%malha
xmin = -4.35; xmax = 4.65; m = 900;

%dados do problema
b   = 0.3; hu   = 0.25; hd = 0; vu = 0; vd = 0;
dam = 0;   n    = 0.005;

T = 41.84; t   = 0; tmax = T/sqrt(g/hu); I = linspace(2,m-1,m-2);
...
figure(1)
plot(x(I)/hu,(U(1,I)+zb(I))/hu,'b',x(I)/hu,zb(I)/hu,'k')
xlim([xmin/hu xmax/hu]), ylim([0 1])
...
function [x,zb,zL,zR,U] = start
    ...
    elev = 35/7.5; heig = 0.075; base = 0.3;
    
    xdam = 1.53; x1 = xdam+elev*heig; x2 = x1+base;
    x3   = x2+elev*heig;
    ...
    for i = 2:m+1
        ...
        if xL >= xdam & xL <= x1 
            zL(i) = -0.327857+0.214286*xL;
        elseif xL > x1 & xL < x2
            zL(i) = heig;
        elseif xL >= x2 & xL <= x3
            zL(i) = 0.542143-0.214286*xL;
        else
            zL(i) = 0;
        end
        
        if xR >= xdam & xR <= x1 
            zR(i) = -0.327857+0.214286*xR;
        elseif xR > x1 & xR < x2
            zR(i) = heig;
        elseif xR >= x2 & xR <= x3
            zR(i) = 0.542143-0.214286*xR;
        else
            zR(i) = 0;
        end
...
function output(x,zb,U,v)
    ...
    outfile = [x/hu;U(1,:)/hu;U(2,:);v;zb];
    fileID = fopen('bs_Godunov.out','w');
